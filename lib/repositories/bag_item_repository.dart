...
class BagItemRepository extends Repository {
  Future<int> addBagItem(
      {int bag_id, int product_id, int quantity}) async {
...
/*
  Future<int> changeQuantity({int bag_item_id, int quantity}) async {
    var response = await httpManager.put(
        url: '/api/bag-item/$bag_item_id',
        data: {'quantity': quantity});
    var total = response['total'];
    return int.parse(total);
  }

  Future<Map> deleteItem({int bag_item_id}) async {
    var response =
    await httpManager.delete(url: '/api/bag-item/$bag_item_id');
